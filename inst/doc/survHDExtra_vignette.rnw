...
First,  one typically has to load or source a function or package which performs the actual model fitting. The next step is the processing of the inputs. In our case we do not use any additional covariates,  so we only have to convert {\bf Xlearn} and {\bf Ylearn} into the input format of the function {\bf rsf}. As can be seen from the code,  this function accepts a {\bf formula} and a {\bf data.frame} containing all necessary variables. Moreover,  we pass all the arguments represented by {\bf \dots} argument and eventually call the function {\bf rsf} using {\bf do.call}. After this the model fitting is complete.

In order to provide outputs which can be evaluated and processed by {\bf survHD}
 we also need a {\bf predict} function. This function should either provide an object of class {\bf LinearPrediction} or {\bf SurvivalProbs} whose slots can be found below:
...
\end{Schunk}
 ...
 \begin{itemize}
 \item {\bf object:} an object of class {\bf ModelCustom} which contain is created by the {\bf survHD} automatically and contains the fitted model (here: {\bf output.rsf}) in its slot {\bf mod}.
