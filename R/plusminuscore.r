...
###
###  modeltype="compoundcovariate": also supports the Compound Covariate method, where coefficients
###  are equal to the univariate Cox regression coefficient.
###
###  modeltype="voting": Each feature receives one vote for positive or negative risk.
###  
###  modeltype="positiveriskvoting": Each feature can only vote for positive risk (poor prognosis) or abstain.
###  
###  modeltype="negativeriskvoting": Each feature can only vote for negative risk (good prognosis) or abstain.
###  
###  **************************************************************************
###  

plusMinus <- function( X, y, lambda=NULL, tuningpar="nfeatures",
                      standardize=FALSE, directionality="posneg", ties.method="average",
                      votingthresholdquantile=0.5, modeltype="plusminus")
{
    supported.modeltypes <- c("plusminus", "compoundcovariate", "tscore", "voting", "positiveriskvoting", "negativeriskvoting")
    if(!modeltype %in% supported.modeltypes)
        stop(paste("provided modeltype:", modeltype, "is not one of the currently supported modeltypes:", supported.modeltypes))
    ...
    if (modeltype %in% c("voting", "positiveriskvoting", "negativeriskvoting")){
        ##Note that these three voting schemes are differentiated only in their predict methods.
        if (standardize)
            warning("Standardize would normally be FALSE in a voting scheme; setting standardize=TRUE will weight the votes by the Cox coefficient of each feature.")
