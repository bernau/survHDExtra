setMethod("plot", signature(x = "GeneSel", y = "missing"), function(x, top = 10, 
    ...
    if (!hasArg(main)) 
        ll$main <- paste("Variable importance plot for iteration", substitute(iter))
    if (!hasArg(cex.lab)) 
        ll$cex.lab <- 1.5
    if (length(imp) == 1) {
        ...
        for (i in 1:top) {
            ...
            .characterPlot(chars[i], bb[i], ll$height[i], deltax = 3/top, deltay = deltay, 
                cex = 15/top)
        }
    } else {
        limp <- length(imp)
        ...
        for (i in 1:limp) {
            ...
            for (i in 1:top) {
                ...
                .characterPlot(chars[i], bb[i], ll$height[i], deltax = 4/top, deltay = deltay, 
                  cex = 15/top)
            }
        }
    }
