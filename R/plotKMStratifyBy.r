...
plot=TRUE,...) {
   ...
   cutpoints  = switch(method,
    ...
    if (is.null(cutpoints)) {
        ...
        mstat = maxstat.test(y~linearriskscore,data=data.frame(linearriskscore=linearriskscore),smethod="LogRank", pmethod=method)
...
...) {
    ...
    for (i in 1:length(cutpoints)) strata[linearriskscore <= sort(cutpoints,decreasing=TRUE)[i]] = labels[i+1]
