...
ylab="Survival (%)",main=NULL,censor.at=NULL,show.n.risk=TRUE,n.risk.step=ifelse(show.n.risk,
...
legendpos="bottomleft",npos="bottomleft",HRpos=ifelse(is.null(main),"margintop","topright"),
show.HR=ifelse(length(levels(strata))==2,TRUE,FALSE),show.PV=TRUE,inverse.HR=TRUE,col=NULL,cex.base=1,cex.lab=cex.base,
cex.legend=cex.base, cex.HR=cex.base,
cex.n.risk=cex.base*0.85,cex.n=cex.base,lwd=2,lty=1:length(levels(strata)),...) {
    ...
    if (show.legend) legend(legendpos, legend.labels,lwd=lwd,lty=lty,
        ...
        for (i in 1:ng) {
            text(tmp$x+strwidth(l,cex=cex.n),tmp$y,adj=tmp$adj, label=fit$n[i],
                col=col[i],cex=cex.n)
            l <- paste(l, fit$n[i])
            if (i<ng)text(tmp$x+strwidth(l,cex=cex.n),tmp$y,adj=tmp$adj, label="/ ",
                cex=cex.n)
            l <- paste(l, "/")
        }
    }
    ...
    if (show.HR || show.PV) {
        if (show.HR) {
            ...
            hr = paste("HR ",conf[1],"; 95% CI, ", conf[3], " to ", conf[4], sep="")
        }
        ...
        if (is.null(HRpos) || HRpos=="margintop") mtext(hr,side=3,cex=cex.HR)
            ...
            if (length(HRpos) > 1) { 
                ...
                .corner.label(label=hr, x=HRcoords[1], y = HRcoords[2],cex=cex.HR)        
            }
        }
    }
    ...
    if (show.n.risk) {
        ...
        nrisk <- no.at.risk(Surv(surv,deceased)~strata,
            data.frame(surv=y[,1],deceased=y[,2],strata), rep(TRUE,length(strata)), n.risk.step, 
            floor(usr.xy[2]))
