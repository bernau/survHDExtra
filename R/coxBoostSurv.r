...
setMethod("coxBoostSurv", signature(X = "data.frame", y = "Surv"), function(X, y, 
    learnind, unpenalizedvars = NULL, essential = FALSE, ...) {
    require(CoxBoost)
    ...
    if (is.null(unpenalizedvars) == TRUE) Xlearn <- data.frame(X[learnind, ])
    else Xlearn <- data.frame(unpenalizedvars[learnind, ], X[learnind, ])
    
    ll <- list(...)
    
    ### check that user does not try to specify unpen.index
    if (is.element("unpen.index", names(ll))) 
        stop("Please use the argument \"unpenalizedvars\" for specifying mandatory variables. ")
    ...
    else Xtest <- data.frame(unpenalizedvars[-learnind, ], X[-learnind, ])
    ...
    pred.learn <- predict(output.coxboost, newdata = Xlearn, type = "lp")
...
... ) {
    coxBoostSurv(X = as.data.frame(X, check.names = FALSE), y = y, ... )
})

setMethod("coxBoostSurv", signature(X = "ExpressionSet", y = "Surv"), function(X, y,
... ) {
    coxBoostSurv(X = as.data.frame(t(exprs(X)), check.names = FALSE), y = y, ... )
})

setMethod("coxBoostSurv", signature(X = "ExpressionSet", y = "character"), function(X, y, 
... ) {
    Xdat <- as.data.frame(t(exprs(X)), check.names = FALSE)
    coxBoostSurv(X = Xdat, y = .fetchyFromEset(X, y), ... )
})

setMethod("predictsurvhd", signature(object = "CoxBoostSurv", newdata = "data.frame"), 
