...
setGeneric("plusMinusSurv", function(X, y, ...) standardGeneric("plusMinusSurv"))

setMethod("plusMinusSurv", signature(X = "data.frame", y = "Surv"), function(X, y, 
    learnind, essential = FALSE, modeltype = "plusminus", ...) {
    ...
    ll <- list(...)  #test this alternative to eval(substitute(list(...)))
    ...
    output <- do.call("plusMinus", args = ll[names(ll) %in% good.arg.names])
...
setMethod("plusMinusSurv", signature(X = "matrix", y = "Surv"), function(X, y, ...) {
    plusMinusSurv(X = as.data.frame(X, check.names = FALSE), y = y, ...)
})

setMethod("plusMinusSurv", signature(X = "ExpressionSet", y = "Surv"), function(X, y, ...) {
    plusMinusSurv(X = as.data.frame(t(exprs(X)), check.names = FALSE), y = y, ...)
})

setMethod("plusMinusSurv", signature(X = "ExpressionSet", y = "character"), function(X, 
    y, ...) {
...
setMethod("predictsurvhd", signature(object = "linearriskscore", newdata = "data.frame"), 
    function(object, newdata, type="lp", timegrid = NULL, ...) {
        ...
        if (!length(object@modeltype)) object@modeltype = "compoundcovariate"
        ...
        if (type == "lp") {
            ...
            if (!identical(names(cc), colnames(newdata))) {
                ## can happen if none of the coefficients are found in newdata
                warning("names of coefficients do not match colnames(newdata).  Make sure columns of newdata correspond to variables.")
                ...
                if (object@modeltype == "tscore") {
                  ...
                  if (length(unique(sign(cc))) == 1) 
                    warning("Must have at least one good-prognosis and one bad-prognosis feature to make predictions with the tscore method.  Returning all NA predictions.")
                  pred <- try(-rowttests(newdata, fac = factor(sign(cc)), tstatOnly = TRUE)[, 
                    1])
                } else if (object@modeltype %in% c("voting", "positiveriskvoting", 
                  "negativeriskvoting")) {
                  votingthresholds <- object@votingthresholds
                  if (length(votingthresholds) == 1) 
                    votingthresholds <- structure(rep(votingthresholds, length(cc)), .Names = names(cc))
                  votingthresholds <- votingthresholds[match(names(cc), names(votingthresholds))]
                  if (!identical(all.equal(names(votingthresholds), names(cc)), TRUE)) 
                    stop("length(model@votingthresholds) must be 1 or length(cc).  If length(cc), their names must match.")
                  ...
                  ## multiply by coefficints.  Value above threshold, positive coef -> positive
                  ...
                  if (object@modeltype == "positiveriskvoting") 
                    newdata.dichot[newdata.dichot < 0] <- 0 else if (object@modeltype == "negativeriskvoting") 
                    newdata.dichot[newdata.dichot > 0] <- 0
                  ## predictions are just the sum of the votes:
                  pred <- rowSums(newdata.dichot)
                } else {
                  pred <- try((as.matrix(newdata) %*% cc)[, 1] + intercept)
                }
            ...
            if (length(object@cutpoint) > 0) {
                pred <- cut(pred, breaks = c(-Inf, object@cutpoint, Inf), ordered_result = TRUE, 
                  include.lowest = TRUE)
